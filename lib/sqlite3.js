...
function normalizeMethod2 (fn) {
    return function (filename, dbname) {
        console.log("HEY", filename, dbname);
        var errBack;
        var args = Array.prototype.slice.call(arguments, 2);
        console.log("REM", args);
        if (typeof args[args.length - 1] === 'function') {
            var callback = args[args.length - 1];
            errBack = function(err) {
                if (err) {
        ...
        var backup = new Backup(this, filename, dbname, errBack);
...
Database.prototype.backup = normalizeMethod2(function(backup, params) {
    return params.length
